import type {Dot, Position, Puzzle} from "../types/game";
...
export class PathValidator {
    ...
    public canBacktrackToPosition(
        ...
        const currentDot = this.puzzle.dots.find(
            (dot) =>
                dot.position.x === currentPos.x && dot.position.y === currentPos.y,
        );
        ...
        if (currentDot) {
            ...
            if (previousDot) {
                const previousDotIndex = path.findIndex(
                    (pos) =>
                        pos.x === previousDot.position.x &&
                        pos.y === previousDot.position.y,
                );
    ...
    private getPreviousNumberedDot(currentDotNumber: number): Dot | null {
        ...
        return (
            this.puzzle.dots.find((dot) => dot.number === currentDotNumber - 1) ||
            null
        );
    }
}
