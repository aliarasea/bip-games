import {CANVAS_CONFIG, GAME_CONFIG} from "../constants/config";
...
export class CanvasHelpers {
     ...
     */
    public static getOptimalCanvasSize(gridSize: number): {
        width: number;
        height: number;
    } {
        const totalSize =
            gridSize * GAME_CONFIG.cellSize + CANVAS_CONFIG.padding * 2;
    ...
    public canvasToGrid(canvasPos: Position): Position {
        return {
            x: Math.round(
                (canvasPos.x - CANVAS_CONFIG.padding) / GAME_CONFIG.cellSize,
            ),
            y: Math.round(
                (canvasPos.y - CANVAS_CONFIG.padding) / GAME_CONFIG.cellSize,
            ),
        };
    }
    ...
    public drawAnimatedDot(
        ...
        const canvasPos = this.gridToCanvas(gridPos);
        const pulseRadius =
            baseRadius + Math.sin(animationProgress * Math.PI * 2) * pulseAmount;
    ...
    public clear(): void {
        const totalSize =
            this.gridSize * GAME_CONFIG.cellSize + CANVAS_CONFIG.padding * 2;
    ...
    public drawDirectionalPath(
        ...
        if (showArrow) {
            const angle = Math.atan2(
                toCanvas.y - fromCanvas.y,
                toCanvas.x - fromCanvas.x,
            );
